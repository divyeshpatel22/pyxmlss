...
class XmlssReader(XmlssBase):
	...
	def getActualRowCount (self):
		'''It will return the actual row count after substracting sub rows from the merged rows'''
	...
	def getRow(self,rowIndex):
		'''
			This Method is used to get the row data based on the provided row index.
			Please use the getMergedRow to get the full data of merged row
			Please use the getMergedRowIndexes  to get the indexes of merged rows in current worksheet.
		'''		
	...
	def getMergedRowIndexes(self):
		'''This Method is used to get the indexes of the merged rows in the current worksheet.'''
		lstMergedRows = self._xmlssDoc.xpath("/ss:Workbook/ss:Worksheet[@ss:Name='{0}']/ss:Table/ss:Row[ss:Cell[@ss:MergeDown and position() = 1]]".format(self._xmlssWorkSheetName),namespaces=self._xmlssXPathNameSpaceMap)
		lstMergedRowIndexes = []
		for mergedRowElem in lstMergedRows:		
	...
	def getMergedRow(self,rowIndex):
		'''This method is used to get the nested data of the merged row based on the provided row index.'''
		if self._debug:					
	...
	def getNextRow(self):
		'''To get the next row after use of the getMergedRow and getRow.
			 Other wise it will start from 0th row onwards.
		'''
	...
	def getRows(self,fromRowIndex,toRowIndex):
		'''
				This method is used to get the set of rows base on list of row indexes(Merged and non merged indexes).
				It will give you the final list as data of rows.
		'''
